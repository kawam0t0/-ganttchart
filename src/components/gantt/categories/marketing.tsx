...
export function MarketingGantt({ project, people, onBack }: MarketingGanttProps) {
  ...
  // データを統合
  useEffect(() => {
    console.log("🔄 MarketingGantt: Combining tasks...")
    console.log("📊 Supabase tasks:", supabaseTasks)
    console.log("📊 Sheet tasks for プロモーション系:", sheetTasks["プロモーション系"])
  ...
  const handleRefresh = async () => {
    console.log("🔄 MarketingGantt: Refreshing data...")
    await Promise.all([refetchSupabase(), refetchSheet()])
  }

  // エラー表示
  if (sheetError) {
    console.error("❌ MarketingGantt: Sheet error:", sheetError)
  }
